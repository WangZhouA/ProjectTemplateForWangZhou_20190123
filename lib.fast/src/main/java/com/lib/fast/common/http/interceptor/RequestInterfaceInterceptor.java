...
public class RequestInterfaceInterceptor implements Interceptor {
    ...
    private boolean isIntercept(Chain chain) {
        ...
        for (String matchsStr : requestUrlMatchs) {
            ...
            intercept = requestUrl.matches(matchsStr);
            if (intercept) break;
        }
        //XLog.tag("okhttp").d("url:%s isintercept:%s", request.url().toString(), (intercept == !isReverseIntercept));
        return intercept == !isReverseIntercept;
    }
    ...
    /**创建末尾一致的正则表达式*/
    protected static String createEndWithMatchers(String endWith){
        String regex = "^\\S*"+endWith+"$";
        return regex;
    }

    /**创建前面一致的正则表达式*/
    protected static String createStartWithMatchers(String startWith){
        String regex = "^"+startWith+"\\S*$";
        return regex;
    }

    /**创建包含该内容(至少有一次)正则表达式*/
    protected static String createHasMatchers(String hasStr){
        String regex = "^\\S*"+hasStr+"\\S*$";
        return regex;
    }
}
