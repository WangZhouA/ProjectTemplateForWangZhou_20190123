...
import com.lib.fast.common.http.cache.CacheType;
import com.lib.fast.common.http.cache.Catch.CacheManager;
...
public class PostCacheInterceptor implements Interceptor {
        ...
        if ("POST".equalsIgnoreCase(request.method())) {
            ...
            if (requestHasCache(request) || requestHasCacheContral(request)) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    LOG.OkHttp.d(TAG + "PostCacheInterceptor exception:%s", e.getMessage());
    ...
    private Response addDelineCacheHead(Response response) {
        ...
        if (!responseOnlyCache(response)) {
            LOG.OkHttp.d(TAG + " add deline cache head");
    ...
    private Response addOnlineCacheHead(Response response) {
        ...
        if (!responseHasCacheContral(response)) {
            LOG.OkHttp.d(TAG + "add online cache head");
        ...
        if (cacheResponse == null) {//未读取到缓存信息
            ...
            if (cacheResponse.code() != 504) {//数据未超时
                if (!responseOnlyCache(cacheResponse)) {
                    ...
                    LOG.OkHttp.d(TAG + "not find only-if-cached head and cache response");
    ...
    private Response checkResponseTimeout(Request request, Response response) {
        ...
        if (TextUtils.isEmpty(dateHead)) {
            ...
            if (System.currentTimeMillis() > date.getTime() + timeout) {//数据超时,返回504
                LOG.OkHttp.d(TAG + "response gateway timeout, remove body and head cache.");
    ...
    private void cacheResponse(Request request, String responseBody) throws IOException {
        ...
        LOG.OkHttp.i(TAG + "--> Push Body Cache: %s :Success", url);
    }
    ...
    private Response getCacheResponse(Request request) {
        LOG.OkHttp.i(TAG + "--> Try to Get Body Cache --------");
        String url = request.url().url().toString();
        ...
        if (cacheStr == null) {
            LOG.OkHttp.i(TAG + "<-- Get Body Cache Failure --------");
            return null;
        }
        ...
        LOG.OkHttp.i(TAG + "<-- Get Body Cache: code:%s message:%s url:%s (%sms)", response.code(), response.message(), url, useTime);
        LOG.OkHttp.i(TAG + cacheStr);
        return response;
    }
    ...
    private void cacheResponseHead(Request request, String responseHead) throws IOException {
        ...
        LOG.OkHttp.i(TAG + "--> Push Head Cache: %s :Success", url);
    }
    ...
    private Headers getCacheHeaders(Request request) {
        LOG.OkHttp.i(TAG + "--> Try to Get Cache Headers   --------");
        ...
        String url = request.url().url().toString();
        if (headersJson == null) {
            LOG.OkHttp.i(TAG + "<-- Get Headers Cache Failure --------");
        } else {
            LOG.OkHttp.i(TAG + "<-- Get Headers Cache url:%s", url);
            LOG.OkHttp.i(TAG + new String(headersJson));
        }
    ...
    private Headers getResponseHeadsByJson(String headsJson) {
        ...
        if (!TextUtils.isEmpty(headsJson)) {
            try {
                ...
                LOG.OkHttp.i(TAG + "<-- newInstance Headers Cache-Control:%s", headers.get("Cache-Control"));
            } catch (Exception e) {
                e.printStackTrace();
                LOG.OkHttp.i(TAG + "<-- newInstance Headers Exception:%s", e.getMessage());
            }
        }
        return headers;
    }
